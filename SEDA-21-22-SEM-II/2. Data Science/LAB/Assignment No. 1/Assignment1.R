...
cat("Q. 6 Mean of Ozone column excluding missing values : ", mean(dataset1$Ozone,na.rm = T),"\n\n")

a <- dataset1[dataset1$Ozone > 31 & dataset1$Temp > 90,]
cat("Q. 7 Ozone above 31 and Temp above 90:\n\n ")
print(a)

a1 <- mean(dataset1$Solar.R[dataset1$Ozone>31 & dataset1$Temp>90], na.rm = T)
cat("\n\nQ. 8 Mean of Solar.R ",a1,"\n\n")
...
ans3 <- sum(dataset2$Hair.Color=="Brown" & dataset2$Eye.Color=="Black")
cat("Q. 3) Number of people having Blonde Hair and Black Eyes: ",ans3,"\n\n")

ans4 <- (mean(dataset2$Eye.Color=="Green")) *100
cat("Q. 4) Percentage of the people with green eyes: ",ans4,"\n\n")

ans5 <- (mean(dataset2$Hair.Color=="Red" & dataset2$Eye.Color=="Blue")) *100
...
answer2 <- median(d$germinated[dataset3$Box=="Covered"])
...
cat("BOXPLOT\n\n")
BP<-ggplot(iris,aes(Species,Sepal.Length,fill=Species))+geom_boxplot(outlier.color="red",outlier.shape=4,outlier.size = 4)+theme(legend.position = "none")+ggtitle("Boxplot")+xlab("species")+ylab("sepallength")
print(BP)

cat("SCATTERPLOT\n\n")
SP<-ggplot(murders,aes(population/10^6,total,col=region))+geom_point(size=3)+scale_x_log10()+scale_y_log10()+geom_text(aes(label=abb),size=3,nudge_x=0.050)+labs(title="SCATTERPLOT",x="Population",y="Total")
print(SP)
